...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    walkingSpeedMean := meanSpeed(action, duration) * kmhInMsec
    height = height / cmInM
    return ((walkingCaloriesWeightMultiplier * weight + (math.Pow(walkingSpeedMean, 2) / height) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
    swimmingSpeedMean := swimmingMeanSpeed(lengthPool, countPool, duration)
    return ((swimmingSpeedMean + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight) * duration
}
