...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return ((runningCaloriesMeanSpeedMultiplier *
		MeanSpeed *
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	SecInH := math.Pow(minInH, 2)
	MeanSpeedInMs := meanSpeed(action, duration) * (mInKm / SecInH)
	MeanSpeedInMsSqrt := math.Pow(MeanSpeedInMs, 2)
	// heightInM := height / 100
	return ((walkingCaloriesWeightMultiplier*weight +
		(MeanSpeedInMsSqrt/height)*
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	return (SwimmingMeanSpeed + swimmingCaloriesMeanSpeedShift) * 2 * weight * duration
}
