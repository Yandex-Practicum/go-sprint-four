...
func meanWalkingSpeed(action int, duration float64) float64 {
	...
	distance := meanSpeed(action, duration) / 3.6
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	calories := ((walkingCaloriesWeightMultiplier*weight + (meanWalkingSpeed(action, duration) / height)) * walkingSpeedHeightMultiplier * duration * minInH)
	return calories
}
