...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	// По какой-то необъяснимой для меня причине без новых переменных результат был неверен
	speed := meanSpeed(action, duration) * kmhInMsec
	sqrSpeed := math.Pow(speed, 2.00)
	h := height / cmInM
	w1 := walkingCaloriesWeightMultiplier * weight
	w2 := walkingSpeedHeightMultiplier * weight
	return (w1 + sqrSpeed/h*w2) * duration * minInH
}
