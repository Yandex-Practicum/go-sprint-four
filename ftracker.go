...
func RunningSpentCalories(action int, weight, duration float64) float64 {
    // ваш код здесь
    runMeanSpead := meanSpeed(action, duration)
    runSpentCalories := (runningCaloriesMeanSpeedMultiplier * runMeanSpead * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    walkMeanSpeadInMin := meanSpeed(action, duration) * kmhInMsec
    walkSpentCalories := (walkingCaloriesWeightMultiplier * weight + (math.Pow(walkMeanSpeadInMin, 2) / (height / cmInM)) * walkingSpeedHeightMultiplier * weight) * duration * minInH
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
    // ваш код здесь
    swimMeanSpeadInHr := swimmingMeanSpeed(lenghtPool, countPool, duration)
    swimSpentCalories := (swimMeanSpeadInHr + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
    return swimSpentCalories
}