...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedInSec := meanSpeed(action, duration) * kmhInMsec
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedInSec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
