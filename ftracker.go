...
// duration float64 — длительность тренировки в часах.
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lenghtPool, countPool int) string {
	switch {
	case trainingType == "Бег":
		distance := distance(action)                               // расчет дистанции
		speed := meanSpeed(action, duration)                       // расчет средней скорости
		calories := RunningSpentCalories(action, weight, duration) // расчет калорий при беге
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	case trainingType == "Ходьба":
		distance := distance(action)                                       // расчет дистанции
		speed := meanSpeed(action, duration)                               // расчет средней скорости
		calories := WalkingSpentCalories(action, duration, weight, height) // расчет калорий при ходьбе
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	case trainingType == "Плавание":
		distance := distance(action)                                               // расчет дистанции
		speed := swimmingMeanSpeed(lenghtPool, countPool, duration)                // расчет средней скорости
		calories := SwimmingSpentCalories(lenghtPool, countPool, duration, weight) // расчет калорий при плавании
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	default:
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	total := (runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight /
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	total := (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)/(height/cmInM))*
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	total := (swimmingMeanSpeed(lenghtPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier *
		weight * duration
	return total
}
