...
func RunningSpentCalories(action int, weight, duration float64) float64 {
    speedMean := meanSpeed(action, duration);
    return ((runningCaloriesMeanSpeedMultiplier * speedMean * runningCaloriesMeanSpeedShift) * weight / mInKM * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    speedMean := meanSpeed(action, duration);
    return ((walkingCaloriesWeightMultiplier * weight + (math.Pow(speedMean, 2) / height) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
    speedMean := swimmingMeanSpeed(lengthPool, countPool, duration);
    return (speedMean + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
