...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	call := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return call
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	call := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedInMs, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return call
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	call := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return call
}
