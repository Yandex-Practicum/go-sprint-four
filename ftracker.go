...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	meanSpeedValue := meanSpeed(action, duration)

	return (runningCaloriesMeanSpeedMultiplier * meanSpeedValue * runningCaloriesMeanSpeedShift) *
...
// duration float64 — длительность тренировки в часах.
// weight float64 — вес пользователя.
// height float64 — рост пользователя. [КАК Я ДОЛЖЕН БЫЛ ПОНЯТЬ, В ЧЁМ ЗАДАЁТСЯ РОСТ? ТОЛЬКО ПО СУЩЕСТВУЮЩЕЙ ПЕРЕМЕННОЙ cmInM?]
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {

	doubledMeanSpeedInMetresPerSecond := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	heightInM := height / cmInM // рост пользователя в метрах [КТО ПИСАЛ ТЗ? Незерьёзно, господа и дамы]

	return (walkingCaloriesWeightMultiplier*weight + doubledMeanSpeedInMetresPerSecond/heightInM*
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	...
	meanSpeedSwim := swimmingMeanSpeed(lenghtPool, countPool, duration)

	return (meanSpeedSwim + swimmingCaloriesMeanSpeedShift) *
