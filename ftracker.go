...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Ходьба":
		...
		fmt.Println(action, duration, weight, height)
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedInMetersPerSecond := meanSpeed(action, duration) * kmhInMsec // Скорость В Метрах В Секунду
	SpentCaloriesWalk := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedInMetersPerSecond, 2)/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	SpentCaloriesSwim := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
