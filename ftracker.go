...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	durationMinutes := duration * float64(minInH)
	speed := meanSpeed(action, duration)
	k1 := float64(runningCaloriesMeanSpeedMultiplier)
	k2 := runningCaloriesMeanSpeedShift

	return k1 * speed * k2 * weight * durationMinutes / float64(mInKm)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	heightM := height / float64(cmInM)

	return (k1*weight + (math.Pow(speed, 2)/heightM)*k2*weight) * durationMinutes
}
...
// SwimmingSpentCalories возвращает колcountPoolичество потраченных калорий при плавании.
//
// Параметры:
//
// lengthPool int — длина бассейна в метрах.
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	k1 := swimmingCaloriesMeanSpeedShift            // 1.1
	k2 := float64(swimmingCaloriesWeightMultiplier) // 2
	speed := swimmingMeanSpeed(lengthPool, countPool, duration)

	return (speed + k1) * k2 * weight * duration
}
