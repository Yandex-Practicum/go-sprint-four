...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedInS := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2) / (height / cmInM)
	spentCalories := (walkingCaloriesWeightMultiplier*weight + meanSpeedInS*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return spentCalories
}
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	spentCalories := (swimmingMeanSpeed(lenghtPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return spentCalories
}
