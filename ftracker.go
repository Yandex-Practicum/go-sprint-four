...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// кол-воКалорий=((18 * СредняяСкоростьВКм/ч * 1.79) * ВесСпортсменаВКг / mInKM * ВремяТренировкиВЧасах * minInH)
	averageSpeed := meanSpeed(action, duration)
	//averageSpeed := distance(action) / duration
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	/*  кол-воКалорий=((0.035 * ВесСпортсменаВКг + (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах)
	...
	calories := ((walkingCaloriesWeightMultiplier*weight + (averageSpeedSquare/heightInMeters)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// кол-воКалорий=(СредняяСкоростьВКм/ч + 1.1) * 2 * ВесСпортсменаВКг * ВремяТренеровкиВЧасах
