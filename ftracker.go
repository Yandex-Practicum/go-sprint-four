...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	avgSpeedMInSec := meanSpeed(action, duration) * kmhInMsec
	heightInM := height * cmInM
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(avgSpeedMInSec, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
