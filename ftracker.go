...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// формула расчета расхода колорий при беге
...
const (
	...
	secInH                          = 3600  // кол-во секунд в часе
)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// формула расчета расхода колорий при ходьбе
	//caloriesWolking := (walkingCaloriesWeightMultiplier*weight + (math.Pow((meanSpeed(action, duration)*mInKm/secInH), 2)/height*walkingSpeedHeightMultiplier*weight)*duration*minInH)
	caloriesWolking := ((walkingCaloriesWeightMultiplier*weight + (math.Pow((meanSpeed(action, duration))*kmhInMsec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return caloriesWolking
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// формула расчета потраченных колорий при плавании
	caloriesSwimming := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
