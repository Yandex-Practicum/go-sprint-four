...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	avgSpeedKmPerHour := meanSpeed(action, duration)
	spentCalories := (runningCaloriesMeanSpeedMultiplier * avgSpeedKmPerHour * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	avgSpeedMetresPerSec := meanSpeed(action, duration) * kmhInMsec
	heightInCm := height/cmInM

	spentCalories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(avgSpeedMetresPerSec, 2)/heightInCm)*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return spentCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	avgSpeedKmPerHour := swimmingMeanSpeed(lengthPool, countPool, duration)
	spendCalories := (avgSpeedKmPerHour + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
