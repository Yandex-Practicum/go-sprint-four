...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	spentRunCalories := (runningCaloriesMeanSpeedMultiplier * avgSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	return spentRunCalories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	avgSpeedInMsec := meanSpeed(action, duration) * kmhInMsec
	heightInCm := height / cmInM
	spentWalkCalories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(avgSpeedInMsec, 2)/heightInCm)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return spentWalkCalories
}
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	avgSpeed := swimmingMeanSpeed(lenghtPool, countPool, duration)
	spentSwimCalories := (avgSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return spentSwimCalories
}
