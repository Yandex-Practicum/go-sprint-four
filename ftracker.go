...
const (
	runMultiplier = 18   // множитель средней скорости.
	runShift      = 1.79 // среднее количество сжигаемых калорий при беге.
)
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return ((runMultiplier * speed * runShift) * weight / mInKm * duration * minInH)

}

// Константы для расчета калорий, расходуемых при ходьбе.
const (
	weightMultiplier = 0.035 // множитель массы тела.
	heightMultiplier = 0.029 // множитель роста.
)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedInHour := meanSpeed(action, duration)
	speedInSec := speedInHour * kmhInMsec
	return ((weightMultiplier*weight + (math.Pow(speedInSec, 2.0)/height*cmInM)*heightMultiplier*weight) * duration * minInH)
}

// Константы для расчета калорий, расходуемых при плавании.
const (
	swimShift      = 1.1 // среднее количество сжигаемых колорий при плавании относительно скорости.
	swimMultiplier = 2   // множитель веса при плавании.
)
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	return (speed + swimShift) * swimMultiplier * weight * duration
