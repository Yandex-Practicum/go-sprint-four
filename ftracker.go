...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    var heightInM = height / cmInM
    var walkingMeanSpeed = math.Pow((meanSpeed(action, duration) * kmhInMsec), 2)
    return ((walkingCaloriesWeightMultiplier * weight + (walkingMeanSpeed / heightInM) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
}
