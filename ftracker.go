...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return ((runningCaloriesMeanSpeedMultiplier * meanSpeedKmH * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMS, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	meanSpeedKmH := swimmingMeanSpeed(lenghtPool, countPool, duration)
	return (meanSpeedKmH + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
