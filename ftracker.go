...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	if height == 0 || weight == 0 || duration == 0 {
		return 0
	}
	return (walkingCaloriesWeightMultiplier*weight +
		math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)/height*cmInM*walkingSpeedHeightMultiplier*weight) *
		duration * minInH
}
