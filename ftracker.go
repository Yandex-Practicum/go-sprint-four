...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	calcCalories := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration)) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	return calcCalories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	calcCalories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow((meanSpeed(action, duration))*kmhInMsec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calcCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	calcCalories := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return calcCalories
}
