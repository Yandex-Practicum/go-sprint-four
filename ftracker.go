...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	var calories, mSpeed float64
	mSpeed = math.Pow(meanSpeed(action, duration), 2) / 3.6 // скорость в м/с возведенная в квадрат
	calories = ((walkingCaloriesWeightMultiplier*weight + (mSpeed/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
