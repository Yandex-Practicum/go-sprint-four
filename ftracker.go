...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	// ваш код здесь
	switch {
	case strings.ToUpper(trainingType) == "БЕГ":
	...
	case strings.ToUpper(trainingType) == "ХОДЬБА":
		distance := distance(action)                                       // вызовите здесь необходимую функцию
		speed := meanSpeed(action, duration)                               // вызовите здесь необходимую функцию
	...
	case strings.ToUpper(trainingType) == "ПЛАВАНИЕ":
		distance := distance(action)                                               // вызовите здесь необходимую функцию
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	speedMul := float64(runningCaloriesMeanSpeedMultiplier)
	speedShift := runningCaloriesMeanSpeedShift
	speedKmH := distance(action) / duration
	spentCalories := ((speedMul * speedKmH * speedShift) * weight / mInKm * duration * minInH)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	walCal := walkingCaloriesWeightMultiplier
	walSpeed := walkingSpeedHeightMultiplier
	heightInM := height / cmInM
	averageSpeedMsec := meanSpeed(action, duration) * kmhInMsec
	speedMsec2 := math.Pow(averageSpeedMsec, 2)
	spentCalories := ((walCal*weight + (speedMsec2/heightInM)*walSpeed*weight) * duration * minInH)
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	speedShift := swimmingCaloriesMeanSpeedShift
	weightMul := float64(swimmingCaloriesWeightMultiplier)
	averageSpeedKmH := swimmingMeanSpeed(lengthPool, countPool, duration)
	spentCalories := (averageSpeedKmH + speedShift) * weightMul * weight * duration
