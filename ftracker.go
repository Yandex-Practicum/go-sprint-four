...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	// Расчет средней скорости бега
	meanSpeed := meanSpeed(action, duration)

	// Расчет количества потраченных калорий при беге
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	speedMtoS := meanSpeed(action, duration) * kmhInMsec
	calories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMtoS, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	calories := (float64(lenghtPool)*float64(countPool)/mInKm/duration + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return calories
}
