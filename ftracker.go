...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return ((walkingCaloriesWeightMultiplier*weight + math.Pow(meanSpeed(action, duration), 2)/height) / (height / cmInM) * walkingSpeedHeightMultiplier * weight) * duration * minInH
}
