...
func meanSpeed(action int, duration float64) float64 {
	...
	return distance / float64(duration)
}
...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Бег":
		distance := distance(action)
		speed := meanSpeed(action, float64(duration))
		calories := RunningSpentCalories(action, float64(weight), float64(duration))
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	case trainingType == "Ходьба":
		distance := distance(action)
		speed := meanSpeed(action, float64(duration))
		calories := WalkingSpentCalories(action, float64(duration), float64(weight), float64(height))
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	case trainingType == "Плавание":
		distance := distance(action)
		speed := swimmingMeanSpeed(lengthPool, countPool, float64(duration))
		calories := SwimmingSpentCalories(lengthPool, countPool, float64(duration), float64(weight))
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	default:
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	ms := meanSpeed(action, float64(duration))
	return ((runningCaloriesMeanSpeedMultiplier * ms * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	ms := swimmingMeanSpeed(lengthPool, countPool, float64(duration))
	return (ms + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
