...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	return float64((18 * meanSpeed(action, duration) * 1.79) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	return float64(((walkingCaloriesWeightMultiplier*weight + ((math.Pow(meanSpeed(action, duration)/3.6, 2))/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH))
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	return float64((swimmingMeanSpeed(lengthPool, countPool, duration) + 1.1) * 2 * weight * duration)
}
