...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	runningSpeed := (runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm
	runningTime := duration * minInH
	return runningSpeed * runningTime
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedDegree := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	walkingSpeed := walkingCaloriesWeightMultiplier*weight + (speedDegree/(height/cmInM))*walkingSpeedHeightMultiplier*weight
	walkingTime := duration * minInH
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	swimmingSpeed := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier
	return swimmingSpeed * weight * duration
}
