...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKM * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(2, meanSpeed(action, distance))/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
