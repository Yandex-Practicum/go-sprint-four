...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Бег":
		distance := distance(action)                               // преодолел дистанцию, км
		speed := meanSpeed(action, duration)                       // средняя скорость на дистанции, км/ч
		calories := RunningSpentCalories(action, weight, duration) // расход энергии, ккал
	...
	case trainingType == "Плавание":
		distance := distance(action)                                               // преодолел дистанцию, км
		speed := swimmingMeanSpeed(lengthPool, countPool, duration)                // средняя скорость на дистанции, км/ч
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeed := meanSpeed(action, duration) * kmhInMsec // m/s
	calories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed, 2.0)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
