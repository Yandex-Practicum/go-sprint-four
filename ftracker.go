...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	k1 := float64(runningCaloriesMeanSpeedMultiplier)
	k2 := float64(runningCaloriesMeanSpeedShift)
	spentCaloriesRun := ((k1 * meanSpeed(action, duration) * k2) * weight / mInKm * duration * minInH)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedCube := math.Pow((meanSpeed(action, duration) * kmhInMsec), 2)
	k1 := walkingCaloriesWeightMultiplier
	k2 := walkingSpeedHeightMultiplier
	spentCaloriesWalk := ((k1*weight + (meanSpeedCube/(height/cmInM))*k2*weight) * duration * minInH)
	return spentCaloriesWalk
}
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	k1 := swimmingCaloriesMeanSpeedShift
	k2 := float64(swimmingCaloriesWeightMultiplier)
	spentCaloriesSwim := (swimmingMeanSpeed(lenghtPool, countPool, duration) + k1) * k2 * weight * duration
	return spentCaloriesSwim
}
